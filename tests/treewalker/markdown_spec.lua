...
describe("Swapping in a markdown file with h2s at the top:", function()
  ...
  describe("scope_confined", function()
    ...
    it("confines move_down", function()
      h.assert_confined_by_parent(33, 1, 'down')
    end)
...
describe("Movement with the cursor outside any heading (preamble/frontmatter)", function()
  before_each(function()
    tw.setup({ scope_confined = false })
    load_fixture("/markdown-frontmatter.md")
  end)

  h.ensure_has_parser("markdown")

  it("moves down from frontmatter to the first heading", function()
    vim.fn.cursor(2, 1)
    tw.move_down()
    h.assert_cursor_at(8, 1, "# First Heading")
  end)

  it("moves down from preamble text to the first heading", function()
    vim.fn.cursor(6, 1)
    tw.move_down()
    h.assert_cursor_at(8, 1, "# First Heading")
  end)

  it("doesn't move up from frontmatter (no heading above)", function()
    vim.fn.cursor(2, 1)
    tw.move_up()
    h.assert_cursor_at(2, 1)
  end)

  it("doesn't move in from frontmatter", function()
    vim.fn.cursor(2, 1)
    tw.move_in()
    h.assert_cursor_at(2, 1)
  end)

  it("doesn't move out from frontmatter", function()
    vim.fn.cursor(2, 1)
    tw.move_out()
    h.assert_cursor_at(2, 1)
  end)

  it("doesn't swap from frontmatter", function()
