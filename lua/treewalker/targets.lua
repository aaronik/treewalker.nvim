...
function M.out(node)
  ...
  if util.is_markdown_file() then
    ...
    if not level then
      local target_node, target_row = markdown.get_nearest_prev_heading(current_row)
    ...
    elseif level then
      ...
      local target_node, target_row = markdown.get_prev_outer_heading(current_row)
      ...
      elseif level > 1 then
        ...
        for row = 1, current_row - 1 do
          local found_level = markdown.get_markdown_heading_level(row)
...
function M.inn()
  ...
  if util.is_markdown_file() then
    ...
    if level then
      ...
      local target_node, target_row = markdown.get_next_inner_heading(current_row)
      if target_node and target_row then
        return target_node, target_row
      else
...
function M.down()
  ...
  if util.is_markdown_file() then
    ...
    if current_row == 1 and level == 1 then
      ...
      local _, is_next_underline = markdown.get_markdown_heading_level(next_row)
    ...
    if level then
      ...
      local target_node, target_row = markdown.get_nearest_next_heading(current_row)
      if target_node and target_row then
        return target_node, target_row
      end
    end
