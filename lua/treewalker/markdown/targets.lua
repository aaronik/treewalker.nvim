...
local get_section_level = heading.get_section_level
local get_section_heading_row_and_node = heading.get_section_heading_row_and_node
local function get_section_heading_row(section_node)
  local section_row, _ = get_section_heading_row_and_node(section_node)
  return section_row
end
...
function M.get_next_same_level_heading(row)
  ...
  return heading.find_section_matching(root, function(_, section_row, section_level)
...
function M.get_nearest_prev_heading(row)
  ...
  local result_node, result_row = heading.find_section_matching(root, function(_, section_row, _)
...
function M.get_nearest_next_heading(row)
  ...
  return heading.find_section_matching(root, function(_, section_row, _)
...
---@return TSNode | nil, integer | nil, boolean
...
function M.find_down(_current_node, current_row)
  if heading.is_heading(current_row) then
    local target_node, target_row = M.get_next_same_level_heading(current_row)
    if target_node and target_row then
      return target_node, target_row
    end
  else
    local target_node, target_row = M.get_nearest_next_heading(current_row)
    if target_node and target_row then
      return target_node, target_row
    end
  end
...
function M.find_up(_current_node, current_row)
  if heading.is_heading(current_row) then
    local target_node, target_row = M.get_prev_same_level_heading(current_row)
    if target_node and target_row then
      return target_node, target_row
    end
  else
    local target_node, target_row = M.get_nearest_prev_heading(current_row)
    if target_node and target_row then
      return target_node, target_row
    end
  end
...
function M.find_in(_current_node, current_row)
  ...
  local target_node, target_row = M.get_next_inner_heading(current_row)
  if target_node and target_row then
    return target_node, target_row
  end
...
function M.find_out(_current_node, current_row)
  ...
  if current_level then
    local target_node, target_row = M.get_prev_outer_heading(current_row)
    if target_node and target_row then
      return target_node, target_row
    end
  else
